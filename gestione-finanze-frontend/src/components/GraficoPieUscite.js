...
const GraficoPieUscite = ({ annoSelezionatoUscite, selectedCategoriesUscite  }) => {
    const { auth } = useAuth();
    const FILL_COLORS = [
    ...
    useEffect(() => {
        const fetchData = async () => {
          const allCategoriesValid = selectedCategoriesUscite.every(
            (categoria) => categoria && categoria.trim() !== ""
          );
      
          if (annoSelezionatoUscite && allCategoriesValid) {
            const requests = selectedCategoriesUscite.map(async (categoria, index) => {
              const url = `/sommaUscite?categoria=${categoria}&year=${annoSelezionatoUscite}`;
            ...
            try {
              ...
              const data = results.map((result, index) => {
                return {
                  name: selectedCategoriesUscite[index],
...
export default GraficoPieUscite;
